...
export function analyzeDenoPermissionNeeds(command: string): PermissionAnalysis {
  ...
  if (/\b(fetch|websocket|serve|listen|connect|postgres|mysql|redis|s3)\b/.test(lower)) {
    net.add("<host>");
  }
  ...
  if (/\b(os\.|hostname|loadavg|networkinterfaces|uid|gid|memoryusage)\b/.test(lower)) {
    sys = true;
  }
