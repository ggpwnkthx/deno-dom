...
const BLOCKED_PATTERNS: ReadonlyArray<{
  ...
  },
  {
    ...
    message: "Do not install or execute dependencies with npm/pnpm/yarn in this Deno repo.",
  },
  {
    pattern: /^\s*npx(?:\s|$)/,
    message: "Do not use `npx` in this Deno repo. Prefer `deno task`, `deno run`, or a custom OpenCode tool.",
  },
  {
  ...
  },
  {
    ...
    message: "Do not use Jest/Vitest directly in this Deno repo. Prefer `deno test` or a Deno task.",
  },
  {
    pattern: /^\s*tsc(?!\s+--noEmit\s*$)(?:\s|$)/,
    message: "Do not use arbitrary `tsc` commands in this Deno repo. Prefer `deno check`.",
  },
];
...
function rewriteCommand(
  ...
  if (packageRun) {
    return {
      ...
      reason: "Use `deno task` instead of package-manager scripts in Deno repos.",
    };
  }
  ...
  if (npmTest) {
    return {
      ...
      reason: "Use `deno test` instead of package-manager test commands in Deno repos.",
    };
  }
