...
export const SessionChecksPlugin: Plugin = async ({ client, worktree }) => {
  let changed = false;
  let running = false;
  ...
  const runChecks = async () => {
    ...
    try {
      const changedFiles = await collectChangedFiles(worktree);
      ...
      for (const command of commands) {
        const result = await runCommand(command, { cwd: worktree });
        results.push({
