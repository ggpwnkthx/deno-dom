...
export default tool({
  ...
  args: {
    command: tool.schema
      .string()
      .min(1)
      .describe("The command, code snippet, or capability description to analyze"),
  },
  async execute(args) {
    ...
    return [
      ...
      analysis.sys ? "- System information access may be needed." : "- No system information access inferred.",
      analysis.ffi ? "- FFI access may be needed." : "- No FFI access inferred.",
      analysis.run ? "- Subprocess execution may be needed." : "- No subprocess execution inferred.",
      analysis.hrtime ? "- High-resolution time access may be needed." : "- No high-resolution time access inferred.",
      "",
      "### Reminder",
