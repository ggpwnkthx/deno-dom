...
export default tool({
  ...
  async execute(args, context) {
    const changedFiles = await collectChangedFiles(context.worktree);
    const allTests = await scanTestFiles(context.worktree);
    ...
    return runSelectedTests(context.worktree, selection.relatedTests);
  },
});
